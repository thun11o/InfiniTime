...
void QuickSettings::OnButtonEvent(lv_obj_t* object, bool wasLongPressed) {
  if (object == btn2) {
    ...
    const bool wasBleRadioEnabled = settingsController.GetBleRadioEnabled();
    if (wasLongPressed && settingsController.GetNotificationStatus() != Controllers::Settings::Notification::Sleep) {
